...
function realTimeChartMulti() {
  ...
  var version = "0.1.0",
      ...
      margin = { top: 20, bottom: 20, left: 80, right: 30, topNav: 10, bottomNav: 20 },
  ...
  var chart = function(s) {
    ...
    yNav = d3.scaleLinear().domain(yDomain).range([heightNav, 0]);
    ...
    setInterval(function() {
      ...
      var extent = !viewport.empty || viewport.empty() ? xNav.domain() : viewport.extent();
  ...
  chart.yDomain = function(_) {
    ...
    if (svg) {
      ...
      y = d3.scale.linear().domain(yDomain).range([heightNav, 0]);
      // update the y axis
      yAxis.scale(y)(yAxisG);
      // update the y ordinal scale for the nav chart
    //   yNav = d3.scale.ordinal().domain(yDomain).rangeRoundPoints([heightNav, 0], 1);
      yNav = d3.scale.linear().domain(yDomain).range([heightNav, 0]);
    }
    return chart;       
  }
